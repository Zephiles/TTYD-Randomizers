...
extern char NewBero[32];
extern char NewMap[9];
...
namespace mod
{
    ...
    extern "C"
    {
        void getRandomWarp()
        {
            ...
            if (ReloadCurrentScreen)
            {
                if (!ChangedLZ)
                {
                    // Write a temporary string into NextBero to tell the player that the new loading zone hasn't been chosen
                    // yet
            ...
            while (!ConfirmNewMap)
            {
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already Need to manually check for the Elevator Key, as it spawns depending on the Sequence
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the main 3
                    // items in the room already
                    static const uint16_t Check_GSWF_Array[] = {4198, 4216, 5665};
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the Card
                    // Key already
                ...
                else if (managestrings::strcmp_NextMap("aji_12"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {4183, 4214};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Get a new map if currently using the challenge mode, 20 minutes have not passed, and the Sequence is less
                    // than 373
                ...
                else if (managestrings::strcmp_NextMap("bom_00"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {3891, 5658};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item in
                    // the room already
                ...
                else if (managestrings::strcmp_NextMap("dou_07"))
                {
                    ...
                    if (!(ttyd::swdrv::swGet(2980)))
                    {
                        if (SequencePosition != 246)
                        {
                            ...
                            if (ttyd::swdrv::swGet(
                                    2988)) // Should only need to check 2988, as the others cannot be on if 2988 isn't
                            {
                    ...
                    {
                        // Prevent the player from warping to this room if the challenge mode is in use and if they have the
                        // curse and the item in the room already
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item
                    // already
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 3 items
                    // in the room already Must check for the Castle Key individually, as it spawns based on Sequence
                    static const uint16_t Check_GSWF_Array[] = {1503, 5606};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("gon_11"))
                {
                    if (SequencePosition < 56)
                    {
                        if (CheckChallengeModeTimerCutoff())
                        {
                            // Get a new map if currently using the challenge mode, 20 minutes have not passed, and the Sequence
                            // is less than 56
                            continue;
                        }
                        else
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 4 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {1507, 1508, 1509, 5540};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                    ...
                    else if (SequencePosition < 50)
                    {
                        ...
                        else
                        {
                            ...
                            if ((CurrentPartnerOut == 0) ||
                                (CurrentPartnerOut > static_cast<uint8_t>(ttyd::party::PartyMembers::Koops)))
                            {
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the Up
                    // Arrow already
                ...
                else if (managestrings::strcmp_NextMap("gor_04"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {5583, 5584};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the Black
                    // Key already
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 3 items
                    // in the room already Must check for the Shop Key individually, as it spawns based on Sequence
                    static const uint16_t Check_GSWF_Array[] = {5631, 5632};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("gra_06"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {2090, 5633};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("hei_00"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 3 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {1778, 1780, 5598};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("hei_10"))
                {
                    if (SequencePosition <= 31)
                    {
                        ...
                        if (CheckChallengeModeTimerCutoff())
                        {
                            ...
                            {
                                // Prevent being able to fight the Gold Fuzzy if the Sequence is set for it and the player has
                                // both stones
                                continue;
                            }
                            else if (CheckForSingleItemWarpToRoomGSWF(1796))
                            {
                                // Prevent warping to this room if the challenge mode is in use and the player has the item in
                                // the room already
                                continue;
                            }
                        }
                        ...
                        {
                            // Prevent warping to this room if the challenge mode is in use, the player does not have both
                            // stones, and the player has the item in the room already
                            continue;
                        }
                    }
                    else if (CheckForSingleItemWarpToRoomGSWF(1796))
                    {
                        // Prevent warping to this room if the challenge mode is in use and the player has the item in the room
                        // already
                        continue;
                    }
                }
                ...
                else if (managestrings::strcmp_NextMap("jin_00"))
                {
                    ...
                    if (CheckChallengeModeTimerCutoff())
                    {
                        ...
                        {
                            // Get a new map if currently using the challenge mode, 20 minutes have not passed, and the player
                            // has a Hammer upgrade
                            continue;
                        }
                        else if (SequencePosition < 198)
                        {
                            ...
                            else
                            {
                                if (tempLZRandoChallenge)
                                {
                                    // Prevent warping to this room if the player has the Steeple Key already and if they cannot
                                    // currently fight the Atomic Boo
                                    continue;
                                }
                                else
                                {
                ...
                else if (managestrings::strcmp_NextMap("jin_05"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 4 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {2235, 2237, 5549, 5635};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item
                    // already and if they currently cannot open the Boo's Chest
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 5 items
                    // in the room already, or if they cannot currently get the letter but already have the 4 other items Must
                    // check for the letter individually, as it spawns based on Sequence
                    static const uint16_t Check_GSWF_Array[] = {2230, 2239, 2240, 5637};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);

                    if (CheckForMultipleItemsWarpToRoomGSWFs(Check_GSWF_Array, ArraySize))
                    {
                        // Prevent warping to this room if the player cannot currently get the latter, or if they already have
                        // it
                ...
                else if (managestrings::strcmp_NextMap("jon_00"))
                {
                    ...
                    if (CheckChallengeModeTimerCutoff())
                    {
                        // Prevent Bonetail from being chosen if currently using the challenge mode and 20 minutes have not
                        // passed yet
                        TotalPitFloors--;
                    }
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use, both blocks are smashed,
                    // and if they have the Palace Key already
                    static const uint16_t Check_GSWF_Array[] = {4339, 4340, 4362};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the Palace
                    // Key already
                ...
                else if (managestrings::strcmp_NextMap("las_15"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use, both blocks are smashed,
                    // and if they have the Palace Key already
                    static const uint16_t Check_GSWF_Array[] = {4345, 4346, 4366};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("las_30"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {4393, 4394};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item
                    // already and if they already have a follower
                ...
                {
                    // Prevent warping to this room if the challenge mode is in use, the player has gotten the item in the room
                    // already, and if the player does not have either the Red Key nor the Blue Key
                ...
                else if (managestrings::strcmp_NextMap("mri_04"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {2837, 2839};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already and if enemies are no longer spawning in the room
                    static const uint16_t Check_GSWF_Array[] = {2844, 5542};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("mri_09"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 3 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {2852, 2853, 5543};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the Super
                    // Boots already
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item
                    // already and if they have already broken the floor panel
                    static const uint16_t Check_GSWF_Array[] = {2871, 5617};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item in
                    // the room already and if enemies are no longer spawning in the room
                ...
                else if (managestrings::strcmp_NextMap("nok_00"))
                {
                    if (SequencePosition < 26)
                    {
                        ...
                        else
                        {
                            ...
                            uint32_t fbatPreviousInteractionPointer =
                                reinterpret_cast<uint32_t>(ttyd::npcdrv::fbatGetPointer());
                ...
                {
                    // Prevent warping to this room if the challenge mode is in use and the player has the 3 items already and
                    // if they cannot currently use the bed
                    static const uint16_t Check_GSWF_Array[] = {3462, 5559, 5649};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("rsh_06_a"))
                {
                    ...
                    {
                        // Get a new map if currently using the challenge mode, 20 minutes have not passed, and the Sequence is
                        // less than 332
                        if (CheckChallengeModeTimerCutoff())
                        {
                            continue;
                ...
                else if (managestrings::strcmp_NextMap("tik_03"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {1356, 5590};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the item
                    // already and if they have the curse already
                    static const uint16_t Check_GSWF_Array[] = {1352, 5597};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (BossCheck)
                                {
                                    ...
                                    {
                                        // Set the Sequence past 128 to prevent the Rawk Hawk and Koopinator cutscene from
                                        // playing
                ...
                else if (managestrings::strcmp_NextMap("tou_05"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {2508, 5624};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 4 items
                    // in the room already and if they are not currently doing the trouble for the Battle Trunks
                    static const uint16_t Check_GSWF_Array[] = {2521, 2523, 5547, 5626};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
                ...
                else if (managestrings::strcmp_NextMap("win_00"))
                {
                    if (SequencePosition <= 84)
                    {
                        if (CheckChallengeModeTimerCutoff())
                        {
                            // Currently using the challenge mode and 20 minutes have not passed, so don't set the sequence for
                            // the Shadow Sirens
                            if (SequencePosition == 84)
                            {
                ...
                else if (managestrings::strcmp_NextMap("win_05"))
                {
                    // Prevent the player from warping to this room if the challenge mode is in use and if they have the 2 items
                    // in the room already
                    static const uint16_t Check_GSWF_Array[] = {2684, 5611};
                    uint32_t ArraySize = sizeof(Check_GSWF_Array) / sizeof(Check_GSWF_Array[0]);
    ...
    bool CheckIfValidLoadingZone(char *CurrentBero)
    {
        ...
        {
            static const char *LoadingZones[] = {"dokan_6", "dokan_7", "dokan_8", "dokan_9", "dokan_10"};
        ...
        {
            static const char *LoadingZones[] = {"s_bero", "A_dokan_02"};
            uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
        ...
        {
            static const char *LoadingZones[] = {"w_bero_1", "w_bero_2"};
            uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
        ...
        {
            static const char *LoadingZones[] = {"ana_1", "ana_2", "A_ana_1", "A_ana_2"};
            uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
        ...
        else if (managestrings::strcmp_NextMap("las_19"))
        {
            if (SequencePosition < 390)
            {
                ...
                static const char *LoadingZones[] = {"n_bero", "e_bero_2"};
                uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
        ...
        else if (managestrings::strcmp_NextMap("las_23"))
        {
            ...
            {
                // Lower part of the room is not loaded
                static const char *LoadingZones[] = {"n_bero_2", "n_bero_1", "e_bero_2"};
                uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
        ...
        else if (managestrings::strcmp_NextMap("las_25"))
        {
            ...
            {
                // Lower part of the room is not loaded
                static const char *LoadingZones[] = {"n_bero_1", "e_bero_2"};
                uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
    ...
    uint32_t Mod::getRandomLZ(void *scriptContext, uint32_t waitMode)
    {
        ...
        if (LZRando)
        {
            ...
            if (!ChangedLZ)
            {
                ...
                if (LoadingZoneTotal > 0)
                {
                    ...
                    do
                    {
                        ...
                        tempNewLoadingZone =
                            *reinterpret_cast<uint32_t *>(scriptContextLZAddresses + (LoadingZoneToUse * 0x3C));
            ...
            if (!LZRandoChallenge)
            {
                ...
                {
                    static const char *LoadingZones[] = {"w_bero_1", "e_bero_2", "s_bero"};
                    uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
                ...
                {
                    static const char *LoadingZones[] = {"e_bero_1", "n_bero"};
                    uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
                ...
                {
                    static const char *LoadingZones[] = {"w_bero_1", "e_bero"};
                    uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
                ...
                {
                    static const char *LoadingZones[] = {"w_bero_1", "e_bero_1"};
                    uint32_t LoadingZoneArraySize = sizeof(LoadingZones) / sizeof(LoadingZones[0]);
            ...
            // Manually transform into the boat if the current room is tik_20, regardless of whether the challenge mode is being
            // used or not
            ...
            if (CloseCurtainFlag)
            {
                ...
                {
                    // Only close the curtain if GSW(4) is not 21, as this is the value used when loading a file that has just
                    // saved after the credits
...
// Check for specific edits to REL files
                ...
                switch (REL)
                {
                    case eki_REL_ID:
                    {
                        ...
                        if (managestrings::strcmp_NextMap("eki_04"))
                        {
                            ...
                            nopInstructionsInREL(reinterpret_cast<void *>(REL_Pointer + eki04_init_script_offset + 0xE4), 0x18);
                        }
                        break;
                    }
                    case gra_REL_ID:
                    {
                        ...
                        nopInstructionsInREL(reinterpret_cast<void *>(REL_Pointer + gra_party_kill_script_offset), 0x9C);
                        break;
                    }
                    case jin_REL_ID:
                    {
                        // Prevent partners and followers from being removed from the field
                        // Replace most of the function with nops, as all of it needs to be avoided
                        nopInstructionsInREL(reinterpret_cast<void *>(REL_Pointer + jin_party_kill_script_offset), 0x9C);
                        ...
                        if (managestrings::strcmp_NextMap("jin_06"))
                        {
                            ...
                            // Replace the conditional part with nops
                            nopInstructionsInREL(reinterpret_cast<void *>(REL_Pointer + jin06_init_script_offset + 0x270),
                                                 0x18);
                        }
                        break;
                    }
                    case las_REL_ID:
                    {
                        ...
                        if (LZRandoChallenge && managestrings::strcmp_NextMap("las_29"))
                        {
                            ...
                            uint32_t NewSeqAddress =
                                REL_Pointer + las29_sekaiyami2_script_offset + las29_game_over_SQ_on_no_address_offset;
                    ...
                    case tik_REL_ID:
                    {
                        ...
                        if (LZRandoChallenge && managestrings::strcmp_NextMap("tik_01"))
                        {
                            ...
                            nopInstructionsInREL(reinterpret_cast<void *>(REL_Pointer + tik01_init_script_offset + 0x314), 0x4);
                        }
                        break;
                    }
                    case usu_REL_ID:
                    {
                        // Prevent partners and followers from being removed from the field
                        // Replace most of the function with nops, as all of it needs to be avoided
                        nopInstructionsInREL(reinterpret_cast<void *>(REL_Pointer + usu_party_kill_script_offset), 0x9C);
                        break;
                    }
                    default:
                    {
    ...
    void reloadScreen()
    {
        ...
        if ((ButtonInput & ReloadScreenCombo) == ReloadScreenCombo)
        {
            ...
            // Prevent reloading the room if currently getting an item. This prevents the player from reloading the room after
            // opening a chest in order to respawn it closed, while at the same time keeping the item from it.
            ...
            // Prevent reloading the room if the player is currently in a room with a boss. This prevents the player from
            // repeatedly killing the boss for SP and/or points.
            uint32_t SequencePosition = ttyd::swdrv::swByteGet(0);
            ...
            else if (managestrings::strcmp_NextMap("jin_00"))
            {
                ...
                {
                    // If the player just got the Steeple Key, then the Sequence would be set to 198. They could then reload the
                    // room to fight the Atomic Boo, even if 20 minutes have not passed.
                    if (CheckChallengeModeTimerCutoff() && (ttyd::mario_pouch::pouchGetHammerLv() > 1))
                    {
                        // The player is currently able to fight the Atomic Boo and 20 minutes have not passed, so don't allow
                        // reloading the room
                        return;
                    }
                }
    ...
    void resetValuesOnGameOver()
    {
        ...
        {
            // Adjust the Sequence if the player got a Game Over on Magnus 1. This will prevent the Emerald Star cutscene from
            // playing if they happen to warp back there Set the Sequence back one
            ttyd::swdrv::swByteSet(0, 110);
        }
    ...
    extern "C"
    {
        ...
        {
            static const char *CheckMapArray[] = {"aji_13", "usu_01", "jin_05", "jin_07", "jin_11", "eki_05", "gor_02"};
    ...
    extern "C"
    {
        ...
        {
            static const char *CheckMapArray[] = {"tik_20", "dou_05", "dou_11"};
            uint32_t ArraySize = sizeof(CheckMapArray) / sizeof(CheckMapArray[0]);
    ...
    extern "C"
    {
        ...
        {
            static const char *CheckMapArray[] = {"aji_13", "dou_11"};
            uint32_t ArraySize = sizeof(CheckMapArray) / sizeof(CheckMapArray[0]);
    ...
    extern "C"
    {
        char *checkCurrentTextbox(char *currentText)
        {
            ...
            if (LZRandoChallenge)
            {
                bool tempCheckCurrentTextboxFlag =
                    CheckCurrentTextboxFlag; // Prevent CheckCurrentTextboxFlag from being loaded multiple times
    ...
    extern "C"
    {
        bool resetSystemFlag(void *GSWAddress)
        {
            ...
            if (ResetSystemFlag && LZRando)
            {
                ...
                // Only reset if GSW(4) is set to 21, as this is the value used when loading a file that has just saved after
                // the credits
                uint32_t GSW_4 = ttyd::swdrv::swByteGet(4);
                if (GSW_4 == 21)
                {
...
// The function needs to be different for JP
    ...
    void Mod::writeLZRandoAssemblyPatches()
    {
        ...
        *reinterpret_cast<uint32_t *>(JumpOnWater) += 0x1;
        ...
        patch::writeBranch(reinterpret_cast<void *>(BranchBackSpawnPartnerBattle),
                           reinterpret_cast<void *>(SpawnPartnerInBattle + 0x4));

        // Prevent battles occuring when reloading the room
        patch::writeBranch(reinterpret_cast<void *>(PreventBattleOnRespawn),
                           reinterpret_cast<void *>(StartPreventBattleOnRespawn));
        patch::writeBranch(reinterpret_cast<void *>(BranchBackPreventBattleOnRespawn),
                           reinterpret_cast<void *>(PreventBattleOnRespawn + 0x4));
        ...
        patch::writeBranch(reinterpret_cast<void *>(BranchBackCheckCurrentTextbox),
                           reinterpret_cast<void *>(CheckCurrentTextbox + 0x4));
...
// Does not need to be changed for the JP version
        ...
        *reinterpret_cast<uint32_t *>(PowerRushPIconCheck6) = 0x60000000; // nop
        ...
        patch::writeBranch(reinterpret_cast<void *>(BranchBackReplaceJumpFallAnim),
                           reinterpret_cast<void *>(ReplaceJumpFallAnim + 0x4));

        // Randomize the loading zone upon loading a file
        patch::writeBranch(reinterpret_cast<void *>(RandomizeBeroFileLoad),
                           reinterpret_cast<void *>(StartRandomizeBeroOnFileLoad));
        patch::writeBranch(reinterpret_cast<void *>(BranchBackRandomizeBeroOnFileLoad),
                           reinterpret_cast<void *>(RandomizeBeroFileLoad + 0x4));
        ...
        patch::writeBranch(reinterpret_cast<void *>(BranchBackResetSystemFlag),
                           reinterpret_cast<void *>(ResetSystemFlag + 0x4));

        // Adjust the Title Screen timer when the player gets a Game Over and is using the Loading Zone randomizer
        patch::writeBranch(reinterpret_cast<void *>(AdjustTitleScreenTimer),
                           reinterpret_cast<void *>(StartAdjustTitleScreenTimer));
        patch::writeBranch(reinterpret_cast<void *>(BranchBackAdjustTitleScreenTimer),
                           reinterpret_cast<void *>(AdjustTitleScreenTimer + 0x4));

        // Prevent the game from crashing in evt_map_blend_set_flag if no partner is out
        patch::writeBranch(reinterpret_cast<void *>(FixEvtMapBlendSetFlagPartners),
                           reinterpret_cast<void *>(StartFixEvtMapBlendSetFlagPartners));
        patch::writeBranch(reinterpret_cast<void *>(BranchBackFixEvtMapBlendSetFlagPartners),
                           reinterpret_cast<void *>(FixEvtMapBlendSetFlagPartners + 0x4));

        // Fix not being able to interact with stuff if the player reloads the room but then hits a shine sprite block while
        // reloading
        patch::writeBranch(reinterpret_cast<void *>(FixShineBlockSystemLevel),
                           reinterpret_cast<void *>(StartFixShineBlockSystemLevel));
        patch::writeBranch(reinterpret_cast<void *>(BranchBackFixShineBlockSystemLevel),
                           reinterpret_cast<void *>(FixShineBlockSystemLevel + 0x4));
    }
